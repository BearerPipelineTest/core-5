...
Feature: share resources where the sharee receives the share in multiple ways
  ...
  Scenario Outline: Sharing parent folder to group with read permission and its child folder to user with all permissions then check rename operation
    ...
    And user "Brian" has accepted share "/parent" offered by user "Carol"
    And user "Alice" has accepted share "/parent" offered by user "Carol"
    When user "Brian" moves file "/Shares/child1/child2/textfile-2.txt" to "/Shares/child1/child2/rename.txt" using the WebDAV API
    Then the HTTP status code should be "201"
    And as "Brian" file "/Shares/parent/child1/child2/rename.txt" should exist
    When user "Brian" moves file "/Shares/parent/child1/child2/rename.txt" to "/Shares/parent/child1/child2/rename2.txt" using the WebDAV API
    Then the HTTP status code should be "403"
    And as "Alice" file "/Shares/parent/child1/child2/rename.txt" should exist
    When user "Alice" moves file "/Shares/parent/child1/child2/rename.txt" to "/Shares/parent/child1/child2/rename2.txt" using the WebDAV API
    Then the HTTP status code should be "403"
  ...
  Scenario Outline: Sharing parent folder to group with read permission and its child folder to user with all permissions then check delete operation
    ...
    And user "Brian" has accepted share "/parent" offered by user "Carol"
    And user "Alice" has accepted share "/parent" offered by user "Carol"
    When user "Brian" deletes file "/Shares/child1/child2/textfile-2.txt" using the WebDAV API
    Then the HTTP status code should be "204"
    And as "Brian" folder "/Shares/parent" should exist
    When user "Brian" deletes folder "/Shares/parent/child1" using the WebDAV API
    Then the HTTP status code should be "403"
    And as "Alice" folder "/Shares/parent" should exist
    When user "Alice" deletes folder "/Shares/parent/child1" using the WebDAV API
    Then the HTTP status code should be "403"
    And as "Alice" folder "/Shares/parent/child1" should exist
    @skipOnOcV10.6 @skipOnOcV10.7 @skipOnOcV10.8.0 @issue-ocis-2440
    Examples:
      | path    |
      | /child1 |
  ...
  Scenario Outline: Sharing parent folder to group with read permission and its child folder to user with all permissions then check reshare operation
    ...
    And user "Brian" has accepted share "/parent" offered by user "Carol"
    And user "Alice" has accepted share "/parent" offered by user "Carol"
    When user "Brian" shares folder "/Shares/child1" with user "Alice" with permissions "read" using the sharing API
    Then user "Alice" should be able to accept pending share "/Shares/child1" offered by user "Brian"
  ...
  Scenario Outline: Share receiver renames a group share and receives same resource through user share with less permissions
    ...
    And user "Brian" has accepted share "/parent" offered by user "Alice"
    And user "Brian" has moved folder "/Shares/parent" to "/Shares/sharedParent"
    When user "Alice" shares folder "parent" with user "Brian" with permissions "read" using the sharing API
    # Note: Brian has already accepted the share of this resource as a member of "grp".
    #       Now he has also received the same resource shared directly to "Brian".
    #       The server should effectively "auto-accept" this new "copy" of the resource
    #       and present to Brian only the single resource "Shares/sharedParent"
